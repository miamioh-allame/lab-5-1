pipeline {
   ...
   post {
    success {
        slackSend("good", "Build Completed: ${env.JOB_NAME} ${env.BUILD_NUMBER}")
    }
    unstable {
        slackSend("warning", "Build Completed: ${env.JOB_NAME} ${env.BUILD_NUMBER}")
    }
    failure {
        slackSend("danger", "Build Completed: ${env.JOB_NAME} ${env.BUILD_NUMBER}")
    }
}
